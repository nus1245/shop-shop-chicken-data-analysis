...
가설·분석 방향(어떤 통계 기법을 쓸지, 결과를 어떻게 해석할지)은 직접 설계했고, 통계 기법의
코드 구현은 AI 코드 어시스턴트(Claude)와 논의하며 기초 골격을 빠르게 잡은 뒤, 프로젝트
데이터의 소표본·요일별 구조에 맞게 직접 발전시켰습니다. 모든 결과는 원본 실행 결과와
대조해 검증했으며, 이 과정에서 AI가 만든 수치가 실제 코드 출력과 다른 것을 발견해 직접
바로잡은 경험도 있습니다.

## 작성자

조승훈 — 데이터 분석가 지망 · [이메일/LinkedIn 등 연락처를 여기에 추가하세요]
