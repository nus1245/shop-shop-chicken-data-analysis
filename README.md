...
OCR로 자동 파싱해 MySQL에 적재하고, 통계적 가설검증과 회귀분석 및 Tableau
대시보드로 전달하는 전체 파이프라인(AtoZ)활동을 혼자 설계·구현했습니다.

| 항목 | 내용 |
|---|---|
| 기간 | 2026.05 ~ 2026.08.21 |
| 데이터 | 배달 플랫폼 주문 영수증 (일 20~35건) |
...
## 핵심 발견
- **H1 기각**: 요일별(월화수) 소비량 차이가 유의 하지않다.(F=2.585, p=0.08 ANOVA)
- **H2 채택**: 특수일(공휴일 및 휴일전) 소비가 평일 소비보다 유의하게 많음 (t=-2.109, p=0.0409, hedges's d=0.63 T-TEST)
- **H3 채택**: 오후(16-19시) 소비가 저녁(20-22시)보다 유의하게 많음 (t=3.605, p=0.0008, Cohen's d=0.53 PAIRED T-TEST)
- **요일별 분리 모델의 함정**: 화요일 단독 스텝와이즈 OLS는 Adj R²=0.942로 매우 높게 나왔지만,
  ...
  위험성을 재확인함
- **통합 모델 채택**:
-  **T1 모델 (1차 피크, 16-19시)**: baseline(단순 평균) 대비, 3일 평균 저녁 소비량·격주 전 같은요일
오후 소비량·특수일(연휴전날) 여부·3일 평균 오후 소비량을 피처로 한 회귀모델 구축
(R²=0.398, Adj R²=0.301, F=4.13, p=0.011). 8월 홀드아웃(n=9) 검증 결과 baseline 대비
RMSE 16.6% 개선
  - 3일 평균 저녁 소비량 1kg↑ → 오후 +746g (p=0.016)
  - 격주 전 같은요일 오후 소비량 1kg↑ → 오후 +364g (p=0.037)
  - 특수일(연휴전날) 여부 → 오후 +1,741g (p=0.087, 경계선)
  - 3일 평균 오후 소비량 1kg↑ → 오후 −541g (p=0.072, 평균회귀 경향)
- **T2 모델 (2차 피크, 20-22:45)**: 5-8월 전체(n=39) 사용, 홀드아웃 없는 탐색적 결과.
`rain_status` 단일 피처로 R²=0.140, p=0.019 — 우천 시(0→1) 저녁 소비 평균 −2,939g 감소.
설명력 14%에 그쳐 저녁 수요의 86%는 현재 변수로 설명되지 않음
...
- **사후 시뮬레이션**: 학습에 쓰지 않은 신규 데이터(n=9 )에 모델을 적용해 사장님의 감각(A),
  모델 예측(B), 단순 일평균(C)을 비교. 
...
└── 07_integerated_model.ipynb           # 최종 모델 
